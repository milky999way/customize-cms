...
const EpisodeList = ({ permissions, ...props }) => {
  // const dataProvider = useDataProvider();
  // console.log(dataProvider);
  const refresh = useRefresh();
  const [pageLoaded, setPageLoaded] = useState(false);
  const [dataSyncArr, setDataSyncArr] = useState([]);
  ...
  useEffect(() => {
    if (!pageLoaded) {
      invalidationCheckRc('none/none/none');
      invalidationCheckLive('none/none/none');
    }
    setPageLoaded(true);
  });
  ...
  ListActions.defaultProps = {
    ...
    onInvalidation: (target, e) => {
      ...
      if (answer) {
        ...
        fetch('/api/episodes/invalidationCache/'+target, {
          method: "POST",
  ...
  const invalidationCheckRc = (paramId) => {
    fetch('/api/episodes/invalidationCheckRc/' + paramId)
    .then(response => {
      return response.json();
    }).then(result => {
      if (result) {
  ...
  const invalidationCheckLive = (paramId) => {
    fetch('/api/episodes/invalidationCheckLive/'+paramId)
    .then(response => {
      return response.json();
    }).then(result => {
      if (result) {
  ...
  const PublishField = (props) => {
    ...
    // return <Button label={"Publish, id:"+props.record.id+", index"+props.record.index} startIcon={<PublishIcon />} className={classes.publishButton} onClick={(e) => onPublish(props.record.id, e)} />;
  ...
  const onPublish = (recordId, e) => {
    e.stopPropagation();
    const answer = window.confirm("해당 에피소드를 live환경에 저장하시겠습니까?");
    if (answer) {
      fetch('/api/episodes/publishEpisode/' + recordId, {
        method: "POST",
        headers: { "Content-Type": "application/json" }
      }).then(response => {
        refresh();
  ...
  return (
    <List {...props} sort={{ field: 'index', order: 'DESC' }} actions={<ListActions />} bulkActionButtons={<BulkActionButtons permissions={permissions} />} title="xxx - Episodes">
      <Datagrid rowClick="show">
        {/* <TextField source="id" /> */}
        <TextField source="index" />
        <TextField source="link" />
        <TextField source="epNum" />
        <TextField source="epIndex" />
        <TextField source="dataEvt" />
        <TextField source="slides" />
        <TextField source="lastSlide" />
        {permissions !== undefined && (permissions.indexOf('RESOURCE:::Administrator') > -1 || permissions.indexOf('RESOURCE:::Episode-Writer') > -1) ?
          <EditButton />
        : null}
        {permissions !== undefined && (permissions.indexOf('RESOURCE:::Administrator') > -1 || permissions.indexOf('RESOURCE:::Episode-Writer') > -1) ?
        ...
        {/* <BooleanField source="dataSync" label="Data Sync (rc / live)" style={{ marginLeft: '25%' }} sortable={false} /> */}
      </Datagrid>
    </List>
  );
};
