...
import { Edit, FormWithRedirect, TextInput, SaveButton, required, RadioButtonGroupInput, NumberInput, useDataProvider, TopToolbar, ListButton, ShowButton, DeleteButton, useEditController } from 'react-admin';
import { Typography, Box, Toolbar } from '@material-ui/core';
import EpisodeImgFieldDialog from './EpisodeImgFieldDialog';



const EpisodeEditActions = ({ basePath, data, resource }) => (
  <TopToolbar>
...
const EpisodeEdit = (props) => {
  ...
  let setupLastSlide;
  record !== undefined ? setupLastSlide = record.lastSlide : setupLastSlide = 'game';

  const [stateLang, setStateLang] = useState(true); // 초기 언어유무 세팅(loop 방지)
  const [stateLangField, setStateLangField] = useState([]); // 초기 언어필드 세팅
  const [stateLastSlide, setStatelastSlide] = useState(setupLastSlide); // 초기 마지막 슬라이드 타입 세팅(state)
  ...
  // console.log(props)
  return (
    <Edit {...props} actions={<EpisodeEditActions />}>
